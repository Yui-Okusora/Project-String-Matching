...
rabinKarp(const vector<vector<char>>& grid, const vector<string>& keywords) {
    ...
    double prepTime = chrono::duration<double, std::milli>(endPrep - startPrep).count();
    ...
    for (size_t k = 0; k < keywords.size(); ++k) {
        ...
        for (int r = 0; r < R; r++) { searchRay(r, 0, 0, 1); searchRay(r, C - 1, 0, -1); }
        for (int c = 0; c < C; c++) { searchRay(0, c, 1, 0); searchRay(R - 1, c, -1, 0); }
        
        for (int c = 0; c < C; c++) searchRay(0, c, 1, 1);
        for (int r = 1; r < R; r++) searchRay(r, 0, 1, 1); 
        
        for (int c = 0; c < C; c++) searchRay(R - 1, c, -1, -1);
        for (int r = 0; r < R - 1; r++) searchRay(r, C - 1, -1, -1); 
        
        for (int c = 0; c < C; c++) searchRay(0, c, 1, -1);
        for (int r = 1; r < R; r++) searchRay(r, C - 1, 1, -1); 
        
        for (int c = 0; c < C; c++) searchRay(R - 1, c, -1, 1);
        for (int r = 0; r < R - 1; r++) searchRay(r, 0, -1, 1); 
    }

    auto end_search = chrono::high_resolution_clock::now();
    double search_time = chrono::duration<double, std::milli>(end_search - startSearch).count();

    return {results, prepTime, search_time};
}